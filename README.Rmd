...
knitr::opts_chunk$set(results='hide', fig.path='images/r-lesson-')
```
------------
...
* In your terminal (tab on the bottom left), first make sure that you're in your `~/intro-r` directory. If not, navigate there. Then, create a folder named `data` within your newly created working directory.
  (e.g., `~/intro-r/data`)
...
If you keep pressing enter, more `+` will appear. To fix this, you can close the parentheses with `)` to evaluate the expression, or <kbd>Esc</kbd> or `Ctrl-C` to escape and leave the expression unevaluated.
